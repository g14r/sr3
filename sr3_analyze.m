...
%
%                       [D] = sr3_analyze('repEffect_ET');                      %group          analysis of repetition effect on execution time (ET)
%                       [D] = sr3_analyze('repEffect_ET', 's01');               %single subject analysis of repetition effect on execution time (ET)
%
...
%
%                       [D] = sr3_analyze('repEffect_RT');                      %group          analysis of repetition effect on execution time (RT)
%                       [D] = sr3_analyze('repEffect_RT', 's01');               %single subject analysis of repetition effect on execution time (RT)
%
...
rs = 0;%0:6; %[0,1,2,3,4,5,6]; %default repetition number subset (full range = 0:10; 0 = full set)
maxRep = 4; %3; % default ceiling level for repetition number (0 = no ceiling)
...
isrepsty = style.custom({lightgray, darkgray}, 'markersize',ms, 'linewidth',lw);
lightgraysty = style.custom({lightgray}, 'markertype','none', 'linewidth',1);
darkgraysty = style.custom({darkgray}, 'markersize',ms, 'linewidth',lw);
...
nm1sty = style.custom({cbs_pink, cbs_blue}, 'markersize',ms, 'linewidth',lw);
...
switch (what)
    ...
    case 'all_data' % pre-analysis: create .mat file containing data from all subjects
        ...
        for s = 1:ns
            ...
            %-------------------------------------------------------------------------------------------------------------------------------------
            % add N-1 info
            D.nm1 = zeros(numel(D.repType), 1); % n-1 exeType flag: 1=go, 0=nogo
            D.nm1(D.repType==111) = 1+1;        % seq rep,    n-1 go,       go
            D.nm1(D.repType==011) = 1+1;        % seq switch, n-1 go,       go
            D.nm1(D.repType==101) = 0+1;        % seq rep,    n-1 nogo,     go
            D.nm1(D.repType==001) = 0+1;        % seq switch, n-1 nogo,     go
    ...
    case 'repEffect_ET' % analysis of repetition effect on ET
        ...
        %[x_coord, y, e] = barplot(T.isRep, T.normET, 'linewidth',lw, 'errorwidth',lw, 'capwidth',0);
        plt.box(T.isRep, T.normET, 'style',boxplotsty);
        
        % stats
        ttest(T.ET(T.isRep==0), T.ET(T.isRep==1), 2, 'paired');
        %[t, p] = ttest(T.ET(T.isRep==0), T.ET(T.isRep==1), 2, 'paired');
        %         text(x_coord(1), max(y)+sum(e)*10, sprintf('t = %2.2f, p < %2.3f', t, p), 'fontsize',fs);
        
        %D1 = D; D1.repNum(D1.repNum >= 1) = 1; %D1.repNum(D1.repNum == 1) = x_coord(2); D1.repNum(D1.repNum == 0) = x_coord(1);
        %T = tapply(D1, {'SN', 'repNum'}, {D1.ET, 'nanmean', 'name', 'ET'}, 'subset', D1.isError==0 & D1.timingError==0 & D1.exeType==1); T = normData(T, {'ET'}, 'sub');
        T.isRep(T.isRep == 1) = 2; T.isRep(T.isRep == 0) = 1;
        hold on;
        plt.line(T.isRep, T.normET, 'plotfcn','median', 'split', T.SN, 'errorbars','none', 'style',lightgraysty, 'leg','off');
        hold on;
        plt.line(T.isRep, T.normET, 'plotfcn','median', 'errorbars','plusminus', 'style',blacksty, 'leg','off');
        ...
        %hold on;
        [~,~] = plt.line(T.repNum, T.normET, 'plotfcn','median', 'errorbars','shade', 'style',darkgraysty, 'leg',{'Group mean'},  'leglocation','northeast');
        xlabel('Repetition number'); ylabel('ET (ms)'); set(gca,'fontsize',fs); axis square; %ylim([690 800]); \
        plt.match('y')
        ...
        %         %hold on;
        %         [~,~] = plt.line(T.BN, (T.normETswc-T.normETrep), 'split',[], 'errorbars','shade', 'style',blacksty, 'leg',{'Group mean'},  'leglocation','southeast');
    ...
    case 'N-1_ET' % analysis of N-1 trial (go/no-go) on repetition effect for ET
        ...
        %             {D.ET, 'nanmean', 'name', 'ET', 'subset', D.nm1==1}, ... %{D.ET, 'nanmean', 'name', 'ET', 'subset', D.repType==1 | D.repType==11}, ...
        ...
        %         % n-1 go
        %         T = tapply(D, {'SN', 'BN', 'repType'}, ...
        %             {D.ET, 'nanmean', 'name', 'ET', 'subset', D.nm1==2}, ...
        ...
        T = tapply(D, {'SN', 'isRep', 'nm1'}, ...
            {D.ET, 'nanmean', 'name', 'ET', 'subset', D.exeType == 1}, ...
        ...
        %         [x, ~, ~] = plt.bar(T.nm1, T.normET, 'split',T.isRep, 'style',isrepsty, 'leg',isrepleg, 'leglocation','northeast');
        ...
        %[x, ~, ~] = plt.bar(T.nm1, T.normET, 'split',T.isRep, 'style',isrepsty, 'leg',isrepleg, 'leglocation','northeast');
        %T.nm1(T.nm1 == 2) = 1; T.nm1(T.isRep == 1) = 0;
        plt.box(T.nm1, T.normET, 'split',T.isRep, 'style',isrepstybox, 'leg',isrepleg, 'leglocation','northeast');
        hold on;
        plt.line([T.nm1 T.isRep], T.normET, 'plotfcn','median', 'split', T.SN, 'errorbars','none', 'style',lightgraysty, 'leg','skip');
        hold on;
        plt.line([T.nm1 T.isRep], T.normET, 'plotfcn','median', 'errorbars','plusminus', 'style',blacksty);
        xlabel('Previous trial'); ylabel('Execution time (ms)'); set(gca,'fontsize',fs); axis square; %ylim([670 830]);
    ...
    case 'repEffect_RT' % analysis of repetition effect on execution time (RT)
        if nargin>1 % load single subj data
            subj = varargin{1};
            D = load( fullfile(pathToData, sprintf('sr3_%s.mat', subj)));
        else % load group data
        ...
        T.isRep(T.isRep == 1) = 2; T.isRep(T.isRep == 0) = 1;
        hold on;
        plt.line(T.isRep, T.normRT, 'plotfcn','median', 'split',[T.SN], 'errorbars','', 'style',lightgraysty, 'leg','off');
        hold on;
        plt.line(T.isRep, T.normRT, 'plotfcn','median', 'errorbars','plusminus', 'style',blacksty, 'leg','off');
        xticklabels({'Switch', 'Repetition'}); ylabel('Reaction time (ms)'); set(gca,'fontsize',fs); axis square; %ylim([410 540]);
        ...
        % stats
        ttest(T.RT(T.repNum==0), T.RT(T.repNum==4), 2, 'paired');
        ttest(T.RT(T.repNum==1), T.RT(T.repNum==4), 2, 'paired');
        ...
        %hold on;
        [~,~] = plt.line(T.repNum, T.normRT, 'plotfcn','median', 'errorbars','shade', 'style',darkgraysty, 'leg',{'Group mean'},  'leglocation','northeast');
        xlabel('Repetition number'); ylabel('RT (ms)'); set(gca,'fontsize',fs); axis square; %ylim([410 540]);
        labels = cell(1, max(D.repNum)+1); labels{1, 1} = 'Switch';
        for l = 1 : max(D.repNum); labels{1, l+1} = num2str(l); end
        labels{1, end} = sprintf('%s+', labels{1, end}); xticklabels(labels);
        plt.match('y');
        ...
        %         %hold on;
        %         [~,~] = plt.line(T.BN, (T.normRTswc-T.normRTrep), 'split',[], 'errorbars','shade', 'style',blacksty, 'leg',{'Group mean'},  'leglocation','southeast');
    ...
    case 'N-1_RT' % analysis of N-1 trial (go/no-go) on repetition effect for RT
        ...
        %         T = tapply(D, {'SN', 'BN', 'repType'}, ...
        %             {D.RT, 'nanmean', 'name', 'RT', 'subset', D.nm1==1}, ... , D.repType==1 | D.repType==11}, ...
        %             'subset', D.isError==0 & D.timingError==0);
        %
        %         % normalize MT data to remove between-subject variability (i.e. plot within-subject standard error)
        %         T = normData(T, {'RT'}, 'sub');
        ...
        %         T = tapply(D, {'SN', 'BN', 'repType'}, ...
        %             {D.RT, 'nanmean', 'name', 'RT', 'subset', D.nm1==2}, ... , D.repType==101 | D.repType==111}, ...
        %             'subset', D.isError==0 & D.timingError==0);% & D.repNum<=1);
        %
        %         % normalize MT data to remove between-subject variability (i.e. plot within-subject standard error)
        %         T = normData(T, {'RT'}, 'sub');
        ...
        % bar plot
        T = tapply(D, {'SN', 'isRep', 'nm1'}, ...
            {D.RT, 'nanmean', 'name', 'RT', 'subset', D.exeType == 1}, ...
            'subset', D.isError==0 & D.timingError==0 & D.nm1>0);
        ...
        subplot(2,2,3); title('ANOVA'); hold on;
        %[x, ~, ~] = plt.bar(T.nm1, T.normRT, 'split',T.isRep, 'style',isrepsty, 'leg',isrepleg, 'leglocation','northeast');
        ...
        hold on;
        plt.line([T.nm1 T.isRep], T.normRT, 'plotfcn','median', 'split', T.SN, 'errorbars','none', 'style',lightgraysty, 'leg','skip');
        hold on;
        plt.line([T.nm1 T.isRep], T.normRT, 'plotfcn','median', 'errorbars','plusminus', 'style',blacksty);
    ...
    case 'FAp_RT' % analysis of False Alarm probability influence on RT (no-go trials only)
        ...
        % create summary table
        T = tapply(D, {'SN', 'isRep'}, ...
            {D.FA, 'nanmean', 'name', 'FAp'}, ...
            {D.RT, 'nanmean', 'name', 'mRT', 'subset', D.isError==0 & D.timingError==0 & D.exeType==1});
    ...
    case 'first2IPIs_RT' % analysis of relationship between first 2 IPIs and RTs (correlation)
        ...
        T=tapply(D, {'SN', 'isRep'},...
            {D.IPI_1, 'nanmean','name','IPI_1'},...
            {D.IPI_2, 'nanmean','name','IPI_2'},...
            {D.IPI_3, 'nanmean','name','IPI_3'},...
            'subset', D.isError==0 & D.timingError==0 & D.exeType==1);
    ...
    case 'N-1_IPI' % analysis of N-1 trial go/no-go on repetition effect, separately for different IPIs
        ...
        %         h = figure; [~,y] = plt.line(T.IPInum, T.normIPI, 'split',T.isRep); close(h);
        %         T.pctIPI = zeros(numel(T.IPI), 1);
        %         T.pctIPI(T.IPInum==1) = 100 - (T.normIPI(T.IPInum==1)  ./  max(y(:,1)) * 100);
        %         T.pctIPI(T.IPInum==2) = 100 - (T.normIPI(T.IPInum==2)  ./  max(y(:,2)) * 100);
        %         T.pctIPI(T.IPInum==3) = 100 - (T.normIPI(T.IPInum==3)  ./  max(y(:,3)) * 100);
        %
        %         %plt.line(T.IPInum, T.pctIPI, 'errorbars','shade','style',darkgraysty, 'leg',isrepleg);
        %         plt.bar(ones(numel(T.pctIPI), 1), T.pctIPI, 'split',T.IPInum, 'leg','auto');
        %         xlabel('IPI number'); ylabel('IPI (ms)'); set(gca,'fontsize',fs);
        %         %xlim([0.5, 3.5]);
        %         ylim([-2, 10]);
        %         axis square;
        ...
        T=tapply(D, {'SN', 'isRep', 'nm1'},...
            ...
            {D.IPI_2, 'nanmean','name','IPI_2'},...
            {D.IPI_3, 'nanmean','name','IPI_3'},...
            'subset', D.isError==0 & D.timingError==0 & D.exeType==1 & D.nm1>0);
        ...
        %         %         [x, ~, ~] = plt.bar(T.isRep, T.normIPI_1, 'split',T.nm1, 'style',nm1sty, 'leg',nm1leg, 'leglocation','northeast');
        %         %         xlabel(''); ylabel('First IPI (ms)'); set(gca,'fontsize',fs); axis square;
        %         %         xticklabels({'Switch', 'Repetition'}); xticks([(x(1)+x(2))/2, (x(3)+x(4))/2]); ylim([150, 250]);
        %         plt.box(T.nm1, T.normIPI_1, 'split',T.isRep, 'style',isrepstybox, 'leg',isrepleg, 'leglocation','northeast');
        %         hold on;
        %         plt.line([T.nm1 T.isRep], T.normIPI_1, 'split', T.SN, 'errorbars','none', 'style',lightgraysty, 'leg','skip');
        %         hold on;
        %         plt.line([T.nm1 T.isRep], T.normIPI_1, 'errorbars','plusminus', 'style',blacksty);
        %         xt = xticks; xticks([sum(xt(1:2))/2 sum(xt(3:4))/2]); xticklabels({'No-go', 'Go'});
        %         xlabel('Previous trial'); ylabel('IPI (ms)'); set(gca,'fontsize',fs); axis square; ylim([130, 340]);
        %
        %         subplot(2,2,3); title('IPI 2'); hold on;
        %         %         [x, ~, ~] = plt.bar(T.isRep, T.normIPI_2, 'split',T.nm1, 'style',nm1sty, 'leg',nm1leg, 'leglocation','northeast');
        %         %         xlabel(''); ylabel('Second IPI (ms)'); set(gca,'fontsize',fs); axis square;
        %         %         xticklabels({'Switch', 'Repetition'}); xticks([(x(1)+x(2))/2, (x(3)+x(4))/2]); ylim([150, 250]);
        %         plt.box(T.nm1, T.normIPI_2, 'split',T.isRep, 'style',isrepstybox, 'leg',isrepleg, 'leglocation','northeast');
        %         hold on;
        %         plt.line([T.nm1 T.isRep], T.normIPI_2, 'split', T.SN, 'errorbars','none', 'style',lightgraysty, 'leg','skip');
        %         hold on;
        %         plt.line([T.nm1 T.isRep], T.normIPI_2, 'errorbars','plusminus', 'style',blacksty);
        %         xlabel('Previous trial'); ylabel('IPI (ms)'); set(gca,'fontsize',fs); axis square; ylim([130, 340]);
        %         xt = xticks; xticks([sum(xt(1:2))/2 sum(xt(3:4))/2]); xticklabels({'No-go', 'Go'});
        %
        %         subplot(2,2,4); title('IPI 3'); hold on;
        %         %         [x, ~, ~] = plt.bar(T.isRep, T.normIPI_3, 'split',T.nm1, 'style',nm1sty, 'leg',nm1leg, 'leglocation','northeast');
        %         %         xlabel(''); ylabel('Third IPI (ms)'); set(gca,'fontsize',fs); axis square;
        %         %         xticklabels({'Switch', 'Repetition'}); xticks([(x(1)+x(2))/2, (x(3)+x(4))/2]); ylim([150, 250]);
        %         plt.box(T.nm1, T.normIPI_3, 'split',T.isRep, 'style',isrepstybox, 'leg',isrepleg, 'leglocation','northeast');
        %         hold on;
        %         plt.line([T.nm1 T.isRep], T.normIPI_3, 'split', T.SN, 'errorbars','none', 'style',lightgraysty, 'leg','skip');
        %         hold on;
        %         plt.line([T.nm1 T.isRep], T.normIPI_3, 'errorbars','plusminus', 'style',blacksty);
        %         xlabel('Previous trial'); ylabel('IPI (ms)'); set(gca,'fontsize',fs); axis square; ylim([130, 340]);
        %         xt = xticks; xticks([sum(xt(1:2))/2 sum(xt(3:4))/2]); xticklabels({'No-go', 'Go'});
        
        %         subplot(2,3,4); title('IPI 1'); hold on;
        %         [x, ~, ~] = plt.bar(T.nm1, T.normIPI_1, 'split',T.isRep, 'style',isrepsty, 'leg',isrepleg, 'leglocation','northeast');
        %         xlabel('N-1'); ylabel('First IPI (ms)'); set(gca,'fontsize',fs); axis square;
        %         xticklabels({'No-go', 'Go'}); xticks([(x(1)+x(2))/2, (x(3)+x(4))/2]); ylim([150, 250]);
        %
        %         subplot(2,3,5); title('IPI 2'); hold on;
        %         [x, ~, ~] = plt.bar(T.nm1, T.normIPI_2, 'split',T.isRep, 'style',isrepsty, 'leg',isrepleg, 'leglocation','northeast');
        %         xlabel('N-1'); ylabel('Second IPI (ms)'); set(gca,'fontsize',fs); axis square;
        %         xticklabels({'No-go', 'Go'}); xticks([(x(1)+x(2))/2, (x(3)+x(4))/2]); ylim([150, 250]);
        %
        %         subplot(2,3,6); title('IPI 3'); hold on;
        %         [x, ~, ~] = plt.bar(T.nm1, T.normIPI_3, 'split',T.isRep, 'style',isrepsty, 'leg',isrepleg, 'leglocation','northeast');
        %         xlabel('N-1'); ylabel('Third IPI (ms)'); set(gca,'fontsize',fs); axis square;
        %         xticklabels({'No-go', 'Go'}); xticks([(x(1)+x(2))/2, (x(3)+x(4))/2]); ylim([150, 250]);
    ...
    case 'sft_ET' % analysis of same finger transition different sequences (sft)
        ...
        %
        T = tapply(D, {'SN', 'sft'}, ...
            {D.ET, 'nanmean', 'name', 'ET', 'subset', D.exeType == 1}, ... & D.sft>=0}, ...
            'subset', D.isError==0 & D.timingError==0);
        ...
        plt.bar(T.sft, T.normET, 'style',blacksty, 'dir','horz');
        xlabel('Execution time (ms)'); ylabel(''); set(gca,'fontsize',fs); axis square;
        yticklabels({'Swc', '0ST', 'S1T', 'S2T', 'S3T', 'S12T', 'S23T', 'Rep'}); xlim([640, 860]);
    ...
    case 'sft_RT' % analysis of same finger transition different sequences (sft)
        ...
        subplot(2,2,1);
        plt.bar(T.sf, T.normRT, 'style',blacksty, 'dir','horz');
        ...
        %         % N-1
        %         T = tapply(D, {'SN', 'sft', 'nm1'}, ...
        %             {D.RT, 'nanmean', 'name', 'RT', 'subset', D.exeType == 1 & D.sft>=0}, ...
        %             'subset', D.isError==0 & D.timingError==0 & D.nm1>0);
        %
        %         % normalize data to remove between-subject variability (i.e. plot within-subject standard error)
        %         T = normData(T, {'RT'}, 'sub');
